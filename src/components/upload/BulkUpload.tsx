        ...
        if (selected === 'programmes' && file) {
            ...
            if (parseData!?.length > 0) {
                for (let i = 0; i < parseData.length; i++) {
                    ...
                    if (!el?.trimester_fee) return swal('Empty Field', `Selected file has an empty field (trimester_fee) on line ${i + 1}. Please check and upload again`, 'warning').then(resetFile)
                    ...
                    const payload = {
                        ...
                        tuition: { amount: el?.tuition, words: el?.amount_in_words, trimester: el?.trimester_fee }
                    }
                    bulkData?.push(payload)
                }
