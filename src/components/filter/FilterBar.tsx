  ...
  return (
    <Box display={'flex'} gap={2}>
        ...
        <Stack direction={'row'} ml={'auto'} alignItems={'center'} gap={1}>
            <IconButton onClick={()=>onViewChange('grid')} sx={{bgcolor: view === 'grid' ? theme=>theme.palette.primary.main : '#ededed', borderRadius: '8px', color: view === 'grid' ? '#fff' : null}}><GridViewIcon size={18} /></IconButton>
            <IconButton onClick={()=>onViewChange('list')} sx={{bgcolor: view === 'list' ? theme=>theme.palette.primary.main : '#ededed', borderRadius: '8px', color: view === 'list' ? '#fff' : null}}><RightToLeftListDashIcon size={18} /></IconButton>
        </Stack>
    </Box>
  )
}
